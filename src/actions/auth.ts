...
export const signUp = async (fullName: string, email: string, password: string) => {
  let sqlDisplay = `EXEC sp_RegisterUser @FullName = N'${fullName}', @Email = '${email}', @PasswordHash = '***'`
  ...
  try {
    ...
    if (existingUser.recordset.length > 0) {
      return { success: false, error: "Email đã được sử dụng.", sqlText: sqlDisplay, result: null }
    }

    const salt = await bcrypt.genSalt(10)
    const passwordHash = await bcrypt.hash(password, salt)

    sqlDisplay = `EXEC sp_RegisterUser @FullName = N'${fullName}', @Email = '${email}', @PasswordHash = '${passwordHash}'`
